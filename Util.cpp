...
QString Util::sizeToString(quint64 size)
{
    ...
    {
        return QObject::tr("%1 KB").arg(QString::number(static_cast<double>(size)/1000.0, 'f', 1));
    }
    else if (size < 1000000000)
    {
        return QObject::tr("%1 MB").arg(QString::number(static_cast<double>(size)/1000000.0, 'f', 1));
    }
    else
    {
        return QObject::tr("%1 GB").arg(QString::number(static_cast<double>(size)/1000000000.0, 'f', 1));
    }
}
